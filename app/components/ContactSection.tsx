...
const contactItems: ContactItemData[] = [
...

    {
        ...
        icon: Home,
        title: "Location",
        description: "1st Floor, Suyog Industrial Estate, 42, Lal Bahadur Shastri Marg, Gandhi Nagar, Vikhroli West, Mumbai, Maharashtra 400083",
...
const ContactItem: React.FC<{ item: ContactItemData }> = ({ item }) => {
    ...
    return (
        <div className="flex items-start space-x-4">
            ...
            <div>
                <h4 className="title text-lg font-semibold text-white">
                    {item.title}
                </h4>
                <p className="icon-box-description text-gray-300">
                    {item.description}
                </p>
            </div>
        </div>
...
const MessagePopup: React.FC<{ message: string | null; onClose: () => void }> = ({
    message,
    onClose,
}) => {
    if (!message) return null;
...
const ContactSection = () => {
    ...
    return (
        <section id="contact" className="py-16 px-4 sm:px-6 lg:px-8 bg-black text-white flex items-center justify-center">
                ...
                <div className="py-8 md:py-0">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-10 gap-x-6 mt-8">
                        {contactItems.map((item) => (
                            <div
                                key={item.id}
                                className={(item.title === "Email" || item.title === "Location") ? "col-span-full" : ""}
                            >
